...
class SendEmailOperator(BaseOperator):
    ...
    def __init__(self,
        ...
        self.html_attached_content = html_attached_content if html_attached_content else []
    ...
    def execute(self, context):
        ...
        if self.pull_xcom_data is not None:
            self.html_reports = context['ti'].xcom_pull(task_ids=self.pull_xcom_data['task_ids'],
                                                        key=self.pull_xcom_data['key'])
        ...
        for html in self.html_reports:
            ...
            html_part.add_header('Content-Disposition', 'attachment', filename=file_name)
